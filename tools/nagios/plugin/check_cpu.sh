...
do_output() {
    output="user:${cpu_user},nice:${cpu_nice},sys:${cpu_sys},iowait:${cpu_iowait},irq:${cpu_irq},softirq:${cpu_softirq},idle:${cpu_idle},cpu_usage:${cpu_usage}"
}

do_perfdata() {
    perfdata="'user'=${cpu_user}%;${warn};${crit};; 'nice'=${cpu_nice}%;${warn};${crit};; 'sys'=${cpu_sys}%;${warn};${crit};; 'softirq'=${cpu_softirq}%;${warn};${crit};; 'iowait'=${cpu_iowait}%;${warn};${crit};; 'irq'=${cpu_irq}%;${warn};${crit};; 'idle'=${cpu_idle}%;${warn};${crit};; 'cpu_usage'=${cpu_usage}%;${warn};${crit};;"
}
...
then
    ...
    then
    echo "$SERVICE WARNING - ${output} | ${perfdata}"
        exit $ST_WR
    elif [ "$cpu_usage" -ge "$crit" ]
    then
        echo "$SERVICE CRITICAL - ${output} | ${perfdata}"
        exit $ST_CR
    else
        echo "$SERVICE OK - ${output} | ${perfdata}"
        exit $ST_OK
    fi
else
    echo "$SERVICE OK - ${output} | ${perfdata}"
