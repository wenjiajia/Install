...
function install_dependency()
{
    ...
    report_to_dialog "$title" "Create databases..." 13
...
" /etc/glance/glance-registry.conf
    ...
    sed -i "s/^filesystem_store_datadir[ ]*=.*/filesystem_store_datadir = \/mnt\/storage\/images/g" /etc/glance/glance-api.conf
...
instances_path=/mnt/storage/instances
...
function install_nova()
{
    ...
    report_to_dialog "$title" "Install virtual compute compute-kvm..." 75
    force_install nova-compute-kvm >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install gcc..." 75
    force_install gcc >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install g++..." 75
    force_install g++ >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install make..." 75
    force_install make >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libtool..." 75
    force_install libtool >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install liblog4cpp5-dev..." 76
    force_install liblog4cpp5-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libavcodec-dev..." 76
    force_install libavcodec-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libssl-dev..." 76
    force_install libssl-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install xlibmesa-glu-dev..." 76
    force_install xlibmesa-glu-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libasound-dev..." 76
    force_install libasound-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libpng12-dev..." 77
    force_install libpng12-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libfreetype6-dev..." 77
    force_install libfreetype6-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libsasl2-dev..." 77
    force_install libsasl2-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libfontconfig1-dev..." 77
    force_install libfontconfig1-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libogg-dev..." 78
    force_install libogg-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libxrandr-dev..." 78
    force_install libxrandr-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install kvm..." 78
    force_install kvm >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libgcrypt-dev..." 78
    force_install libgcrypt-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libsdl-de...v" 78
    force_install libsdl-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libnss3-dev..." 79
    force_install libnss3-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libpixman-1-dev..." 79
    force_install libpixman-1-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libxfixes-dev..." 79
    force_install libxfixes-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libjpeg8-dev..." 79
    force_install libjpeg8-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libsasl2-dev..." 79
    force_install libsasl2-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install python-pyparsing..." 80
    force_install python-pyparsing >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install glib-2.0-dev..." 80
    force_install glib-2.0-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libalsa-ocaml-dev..." 80
    force_install libalsa-ocaml-dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Install libusb-1.0-0.dev..." 80
    force_install libusb-1.0-0.dev >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Upgrade_kvm..." 81
    upgrade_kvm >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Service libvirt-bin restart..." 82
    service libvirt-bin restart >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Virtual compute_config..." 82
    nova_config >> ${LOG_FILE} 2>>${ERR_FILE}
    sleep 1
    report_to_dialog "$title" " Virtual compute-manage db sync..." 82
    nova-manage db sync >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Virtual compute all restart..." 82
    nova_all_restart >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Flavor customize..." 83
    ...
    report_to_dialog "$title" "Copy virtio-win-0.1-15.iso..." 83
    cp ./win/virtio-win-0.1-15.iso /var/lib/nova >> ${LOG_FILE} 2>>${ERR_FILE}
    report_to_dialog "$title" "Copy virtio-WinXP-x86.vfd..." 84
...
function install_cinder()
{
    ...
    sleep 1
    report_to_dialog "$title" "Remove virtual disk service volume..." 52
    apt-get autoremove --purge --yes cinder-volume >> ${LOG_FILE} 2>>${ERR_FILE}
    sleep 1
    report_to_dialog "$title" "Remove iscsitarget..." 52
    ...
    sleep 1
    report_to_dialog "$title" "Remove iscsitarget-dkms..." 52
    ...
    report_to_dialog "$title" "Install virtual disk service-volume..." 55
    ...
    report_to_dialog "$title" "Install iscsitarget-dkms..." 58
    ...
    sleep 1
    report_to_dialog "$title" "Service iscsitarget restart..." 60
...
function iscsi_config(){
    ...
    sed -i "s/[# ]*discovery.sendtargets.auth.authmethod[ ]*=.*/discovery.sendtargets.auth.authmethod = CHAP/g" /etc/iscsi/iscsid.conf
    sed -i "s/[# ]*discovery.sendtargets.auth.username[ ]*=.*/discovery.sendtargets.auth.username = vrv/g" /etc/iscsi/iscsid.conf
    sed -i "s/[# ]*discovery.sendtargets.auth.password[ ]*=.*/discovery.sendtargets.auth.password = vrv/g" /etc/iscsi/iscsid.conf
...
" /etc/cinder/api-paste.ini
    storage_sec_ip=${storage_sec_ip:-10.0.1.3}
...
{
    storage_sec_ip=${storage_sec_ip:-10.0.1.3}
    iscsiadm -m discovery -t sendtargets -p $storage_sec_ip:3260
    iscsiadm -m node -T iqn.2013-08.org.jointlab:cinder -p $storage_sec_ip:3260 --login
    disk_cinder=`fdisk -l | grep /dev/ | awk END'{print $2}' | awk -F":" '{print $1}'`
    echo $disk_cinder > /var/log/disk_cinder
    iscsiadm -m node -T iqn.2013-08.org.jointlab:glance -p $storage_sec_ip:3260 --login
...
function creeper_config(){
    ...
    echo "control_pri_ip = $control_pri_ip"
...
function upgrade_kvm()
{
    ...
    pushd $target/libcacard-0.1.2 && ./configure --quiet && make -s && make -s install
    popd 

    pushd $target/spice-protocol-0.12.2 && ./configure --quiet && make -s && make -s install
    popd

    pushd $target/celt-0.5.1.3 && ./configure --quiet && make -s && make -s install
    popd

    pushd $target/usbredir-0.4.4 && ./configure --quiet && make -s && make -s install
    popd

    pushd $target/spice-0.12.2 && ./configure --quiet --enable-smartcard --enable-client && make -s && make -s install
    popd

    pushd $target/qemu-kvm-1.2.0
    ...
    --enable-vhost-net --enable-vnc --enable-linux-user --enable-mixemu --enable-nptl --enable-pie --enable-sdl
    ...
    #cp /usr/local/bin/qemu-* /usr/bin/
    #rm /usr/local/bin/qemu-*
    #cp /usr/local/bin/vscclient /usr/bin/
    #rm /usr/local/bin/vscclient
